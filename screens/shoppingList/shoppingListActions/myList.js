...
import { DietMainStyles } from "../shoppingListHome/MainStyles";

const MyLists = ({ navigation, props }) => {
  ...
  let getDiets = async () => {
    try {
      const q = query(collection(db, "shLists"));
  ...
  return (
    <Card style={Styles.cardContainer}>
      <Card.Content style={Styles.cardContent}>
        <Text style={StylesLocal.cardTitle}>My Shopping List</Text>
        <TextInput
          // label="Search"
          value={searchQuery}
        ...
        <ScrollView style={Styles.staticTextView}>
          {diets &&
            diets
              .filter((diet) =>
                diet.shlistName.toLowerCase().includes(searchQuery.toLowerCase())
              )
              .map((diet, i) => (
                <TouchableOpacity
                  key={i}
                  onPress={() => navigation.navigate("ViewList", diet)}
                >
                  <Card style={styles.card}>
                    <Card.Content>
                      <View style={styles.cardTitleRow}>
                        <Text style={styles.cardDietName}>{diet.shlistName}</Text>
                        <View style={styles.cardAuthorView}>
                          <Text style={styles.cardDietAuthor}>
                            {getDietUser(diet.shlistUser, diet.shlistUserName)}
                          </Text>
                        </View>
                      </View>
                      {/* <Text>{diet.dietUser}</Text> */}
                      <Text>{diet.shlistDesc}</Text>
                    </Card.Content>
                  </Card>
                </TouchableOpacity>
              ))}
      ...
      <Card.Actions style={DietMainStyles.cardActionsStyle}>
        <Button
          ...
          onPress={() => navigation.navigate("AddList", "Create")}
          //onPress={logger}
        >
          <Text style={DietMainStyles.text}>Create new Shopping List</Text>
        </Button>
      </Card.Actions>
    </Card>
  );
